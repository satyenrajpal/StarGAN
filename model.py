...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=128, conv_dim=64, c_dim=[5], repeat_num=6):
        ...
        self.conv2 = nn.ModuleList([nn.Conv2d(curr_dim, dim, kernel_size=kernel_size, bias=False) for dim in c_dim])
        
    def forward(self, x):
        h = self.main(x)
        out_src = self.conv1(h)
        out_cls = [conv2(h) for conv2 in self.conv2]
        out_cls = [cls_probs.view(cls_probs.size(0), cls_probs.size(1)) for cls_probs in out_cls]
